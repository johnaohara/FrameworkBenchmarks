...
RUN mvn package -q -pl pgclient-reactiverse-custom-pool -am

FROM openjdk:11.0.6-jdk-slim
WORKDIR /quarkus
COPY --from=maven /quarkus/pgclient-reactiverse-custom-pool/target/lib lib
COPY --from=maven /quarkus/pgclient-reactiverse-custom-pool/target/pgclient-reactiverse-cusom-pool-1.0-SNAPSHOT-runner.jar app.jar
