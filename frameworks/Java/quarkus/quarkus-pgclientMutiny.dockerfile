...
RUN mvn package -q -pl  pgclientMutiny -am

FROM openjdk:11.0.6-jdk-slim
WORKDIR /quarkus
COPY --from=maven /quarkus/ pgclientMutiny/target/lib lib
COPY --from=maven /quarkus/ pgclientMutiny/target/pgclient-mutiny-1.0-SNAPSHOT-runner.jar app.jar
