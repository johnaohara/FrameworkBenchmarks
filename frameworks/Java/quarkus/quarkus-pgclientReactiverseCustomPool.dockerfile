...
RUN mvn package -q -pl pgclientReactiverseCustomPool -am

FROM openjdk:11.0.6-jdk-slim
WORKDIR /quarkus
COPY --from=maven /quarkus/pgclientReactiverseCustomPool/target/lib lib
COPY --from=maven /quarkus/pgclientReactiverseCustomPool/target/pgclient-reactiverse-cusom-pool-1.0-SNAPSHOT-runner.jar app.jar
