...
RUN mvn package -q -pl  pgclient-mutiny-iterator -am

FROM openjdk:11.0.6-jdk-slim
WORKDIR /quarkus
COPY --from=maven /quarkus/pgclient-mutiny-iterator/target/lib lib
COPY --from=maven /quarkus/pgclient-mutiny-iterator/target/pgclient-mutiny-iterator-1.0-SNAPSHOT-runner.jar app.jar
